...
export async function getCardDetails(params: GetCardDetailsParams) {
  ...
  try {
    ...
    const boardLabels = Array.isArray(included.labels) ? included.labels : [];
    const cardLabels = Array.isArray(included.cardLabels) ? included.cardLabels : [];
    const assignedLabelIds = new Set(
      cardLabels
        .filter((cardLabel: any) => cardLabel.cardId === card.id)
        .map((cardLabel: any) => cardLabel.labelId),
    );
    const labels = boardLabels.filter((label: any) => assignedLabelIds.has(label.id));
    ...
    const hasRecentHumanFeedback =
      !!firstComment &&
      !firstComment.text.includes("Implemented feature") &&
      !firstComment.text.includes("Awaiting human review");
